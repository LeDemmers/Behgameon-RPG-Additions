...
public class MineSlashHandler {
	...
	public MineSlashHandler() {
		...
		for (int i = 1; i <= 3; i++)
			MineAndSlashAPI.addCompatibleItem(BehgameonMod.MODID + ":accessory_" + i, new ConfigItem()
					.setType(Bracelet.INSTANCE).setSalvagable(Config.SERVER.USE_MINESLASHLOOTSYSTEM.get()));
		...
		for (int i = 33; i <= 44; i++)
			MineAndSlashAPI.addCompatibleItem(BehgameonMod.MODID + ":accessory_" + i, new ConfigItem()
					.setType(Charm.INSTANCE).setSalvagable(Config.SERVER.USE_MINESLASHLOOTSYSTEM.get()));
		...
		for (int i = 49; i <= 54; i++)
			MineAndSlashAPI.addCompatibleItem(BehgameonMod.MODID + ":accessory_" + i,
					new ConfigItem().setType(Necklace.INSTANCE).setAlwaysUnique()
							.setSalvagable(Config.SERVER.USE_MINESLASHLOOTSYSTEM.get()));
